...
test("TS_11_To_verify_that_payment_list_button_in_viewing_page_is_functional", async ({ page }) => {
  const data = loadRuntimeData();
  test.info().annotations.push({
    type: 'source-aiq',
    description: "source-aiq/Test Scripts/imREmit_Lite_Dashboard/Payables_pending_for_MORE_THAN_5_DAYS/TS_11_To_verify_that_payment_list_button_in_viewing_page_is_functional.ds"
  });
  ...
  await test.step('Return from payment details to the payments list', async () => {
    await imremitLiteDashboardPayablesPendingForMoreThan5DaysHelpers.clickDetailButton(
      page,
      imremitLiteDashboardPayablesPendingForMoreThan5DaysHelpers.selectors.detail.paymentList
    );
    await imremitLiteDashboardPayablesPendingForMoreThan5DaysHelpers.assertPaymentManagementVisible(page);
  });
