...
test("TS_24_To_verify_that_the_Edit_Customer_Details_Button_in_Actions_icon_is_functional", async ({ page }) => {
  const data = loadRuntimeData();
  test.info().annotations.push({
    type: 'source-aiq',
    description: "source-aiq/Test Scripts/Customer_Management_Admin/TS_24_To_verify_that_the_Edit_Customer_Details_Button_in_Actions_icon_is_functional.ds"
  });
  ...
  await test.step('Run converted flow', async () => {
    ...
    await page.waitForURL((url) => /\/app\/admin\/customer-management\/.+/.test(url.pathname), {
      timeout: 15000
    });
  });
