...
test("TS_29_To_verify_that_the_Invoice_Char_Diff_criteria_settings_can_be_added", async ({ page }) => {
  const data = loadRuntimeData();
  const criteriaValue = String(Date.now()).slice(-2);
  test.info().annotations.push({
    type: 'source-aiq',
    description: "source-aiq/Test Scripts/Criteria_Settings/TS_29_To_verify_that_the_Invoice_Char_Diff_criteria_settings_can_be_added.ds"
  });
  ...
  await test.step('Add Invoice Char Diff criteria', async () => {
    await criteriaSettingsHelpers.openCriteriaCardAction(page, 'Invoice Char Diff', 'Add Criteria');
