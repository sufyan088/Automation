...
test("TS_15_To_verify_that_the_history_button_in_viewing_payment_page_is_functional", async ({ page }) => {
  const data = loadRuntimeData();
  test.info().annotations.push({
    type: 'source-aiq',
    description: "source-aiq/Test Scripts/imREmit_Lite_Dashboard/Payables_pending_for_MORE_THAN_5_DAYS/TS_15_To_verify_that_the_history_button_in_viewing_payment_page_is_functional.ds"
  });
  ...
  await test.step('Open history in payment details', async () => {
    await imremitLiteDashboardPayablesPendingForMoreThan5DaysHelpers.clickDetailButton(
      page,
      imremitLiteDashboardPayablesPendingForMoreThan5DaysHelpers.selectors.detail.history
    );
  });
