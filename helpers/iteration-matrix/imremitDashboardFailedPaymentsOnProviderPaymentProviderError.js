...
async function openModule(page) {
  await clickIfFound(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.moduleTabs.imremit, {
    mustBeVisible: true,
...
async function openDashboard(page) {
  await clickWithFallback(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.moduleTabs.dashboard, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  });
  await waitForAppToSettle(page, 1000);
...
async function expectOverviewVisible(page) {
  const heading = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.headings.overview, {
    mustBeVisible: true,
...
async function selectCustomer(page, data) {
  ...
  let trigger;
  try {
    trigger = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.customerPicker.trigger, {
      mustBeVisible: true,
      timeoutPerCandidate: 2500
    });
  } catch (error) {
  ...
  if (preferredCustomer) {
    const searchInput = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.customerPicker.searchInput, {
      mustBeVisible: true,
      timeoutPerCandidate: 2500
    }).catch(() => null);

    if (searchInput) {
      await fillWithFallback(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.customerPicker.searchInput, preferredCustomer, {
        mustBeVisible: true,
        timeoutPerCandidate: 2500
      });
      ...
      if (fallbackCustomer) {
        await fillWithFallback(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.customerPicker.searchInput, fallbackCustomer, {
          mustBeVisible: true,
          timeoutPerCandidate: 2500
        });
        await waitForAppToSettle(page, 750);
  ...
  const option = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.customerPicker.options, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  }).catch(() => null);
...
async function openCustomerDetails(page) {
  await clickWithFallback(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.dashboard.viewCustomerDetails, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  });
  await waitForAppToSettle(page, 1000);

  const detailButton = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.dashboard.viewCustomerDetails, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  }).catch(() => null);
...
async function closeCustomerDetails(page) {
  const closeControl = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.dashboard.customerDetailsClose, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  }).catch(() => null);
...
async function openFailedPaymentsList(page, data) {
  ...
  const openPaymentManagement = async () => {
    const reviewAction = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.dashboard.review, {
      mustBeVisible: true,
      timeoutPerCandidate: 2500
    }).catch(() => null);
  ...
  const heading = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.headings.paymentManagement, {
    mustBeVisible: true,
    timeoutPerCandidate: 5000
  });
  await expect(heading.locator).toBeVisible({ timeout: 10000 });
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    const visiblePaymentRowAction = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.list.actionsMenu, {
      mustBeVisible: true,
...
async function ensurePreviousPaymentEnabled(page) {
  const previous = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.previousPayment, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  }).catch(() => null);
  ...
  if (isDisabled) {
    await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.nextPayment);
  }
}

async function addComment(page, commentText) {
  await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.comments);
  await fillWithFallback(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.commentInput, commentText, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  });
  await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.addComment);
  const toast = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.commentSuccessToast, {
    mustBeVisible: true,
    timeoutPerCandidate: 5000
  }).catch(() => null);
  if (toast) {
...
async function openCommentRowActions(page, commentText) {
  const visibleCommentRow = await resolveFirst(
    page,
    imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.commentRow(commentText),
    {
      mustBeVisible: true,
      timeoutPerCandidate: 1500
    }
  ...
  const commentRow = visibleCommentRow || await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.commentRow(commentText), {
    mustBeVisible: true,
    timeoutPerCandidate: 5000
  });
...
async function openActionsMenu(page) {
  await clickWithFallback(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.list.actionsMenu, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  });
  await waitForAppToSettle(page, 500);
...
async function openPaymentDetails(page, data) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const paymentListButtonBeforeOpen = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.paymentList, {
      mustBeVisible: true,
    ...
    const detailsMenuItem = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.list.viewPaymentDetails, {
      mustBeVisible: true,
      timeoutPerCandidate: 1500
    }).catch(() => null);
  ...
  const paymentListButton = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.paymentList, {
    mustBeVisible: true,
    timeoutPerCandidate: 5000
  }).catch(() => null);
...
async function clickDetailButton(page, candidates) {
  const isCommentsButton = candidates === imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.comments;
...
async function clickHeaderAndChoose(page, label, actionCandidates) {
  const initialHeader = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.table.header(label), {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  });
  ...
  const header = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.table.header(label), {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  });
...
async function runTs04(page, data) {
  ...
  await reportStep('Move to the next payment-management page', async () => {
    await clickPagination(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.pagination.next);
  });
}
...
async function runTs05(page, data) {
  ...
  await reportStep('Move to the last payment-management page', async () => {
    await clickPagination(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.pagination.last);
  });
}
...
async function runTs06(page, data) {
  ...
  await reportStep('Move forward and then return to the previous page', async () => {
    await clickPagination(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.pagination.next);
    await clickPagination(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.pagination.previous);
  });
}
...
async function runTs07(page, data) {
  ...
  await reportStep('Move to the last page and return to the first page', async () => {
    await clickPagination(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.pagination.last);
    await clickPagination(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.pagination.first);
  });
}
...
async function runTs08(page, data) {
  ...
  await reportStep('Verify pagination controls are visible', async () => {
    for (const candidates of [
      imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.pagination.first,
      imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.pagination.previous,
      imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.pagination.next,
      imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.pagination.last
    ]) {
      const result = await resolveFirst(page, candidates, {
        mustBeVisible: true,
        timeoutPerCandidate: 2500
...
async function runTs09(page, data) {
  ...
  await reportStep('Export payment data from the list', async () => {
    ...
    await clickWithFallback(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.list.export, {
      mustBeVisible: true,
      timeoutPerCandidate: 2500
    });
...
async function runTs10(page, data) {
  ...
  await reportStep('Open the column view control', async () => {
    await clickWithFallback(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.list.columnView, {
      mustBeVisible: true,
      timeoutPerCandidate: 2500
    });
  });
...
async function runTs11(page, data) {
  ...
  await reportStep('Open the actions menu for a payment row', async () => {
    await openActionsMenu(page);
    const item = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.list.viewPaymentDetails, {
      mustBeVisible: true,
      timeoutPerCandidate: 2500
    });
...
async function runTs13(page, data) {
  ...
  await reportStep('Return to the previous payment from the details view', async () => {
    ...
    await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.previousPayment);
  });
}
...
async function runTs14(page, data) {
  ...
  await reportStep('Return from payment details to the payment list', async () => {
    await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.paymentList);
    const heading = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.headings.paymentManagement, {
      mustBeVisible: true,
      timeoutPerCandidate: 5000
    });
...
async function runTs15(page, data) {
  ...
  await reportStep('Open the edit comment action', async () => {
    ...
    await openCommentRowActions(page, 'Comment');
    await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.editComment);
    await fillWithFallback(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.commentInput, 'Comment_Edit', {
      mustBeVisible: true,
      timeoutPerCandidate: 2500
    });
    await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.saveComment);
  });
}
...
async function runTs16(page, data) {
  ...
  await reportStep('Open the delete comment action', async () => {
    ...
    await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.deleteComment);
    const toast = await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.deleteCommentSuccessToast, {
      mustBeVisible: true,
      timeoutPerCandidate: 5000
    }).catch(() => null);
    if (toast) {
...
async function runTs17(page, data) {
  ...
  await reportStep('Open the Invoices view in payment details', async () => {
    await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.invoices);
  });
}
...
async function runTs18(page, data) {
  ...
  await reportStep('Open the General Information view in payment details', async () => {
    await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.generalInformation);
  });
}
...
async function runTs19(page, data) {
  ...
  await reportStep('Open the Transactions view in payment details', async () => {
    await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.transactions);
  });
}
...
async function runTs20(page, data) {
  ...
  await reportStep('Open the Authorization and Decline view in payment details', async () => {
    await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.authorizationAndDecline);
  });
}
...
async function runTs21(page, data) {
  ...
  await reportStep('Open the History view in payment details', async () => {
    await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.history);
  });
}
...
async function runTs22(page, data) {
  ...
  await reportStep('Open the Block Payment action in payment details', async () => {
    await clickDetailButton(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.detail.blockPayment);
  });
}
...
async function runTs23(page, data) {
  ...
  await reportStep('Apply ascending order from visible table headers', async () => {
    for (const label of ['Supplier Name', 'Org Id', 'Facility Name', 'Payment Number']) {
      await clickHeaderAndChoose(page, label, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.menus.ascending);
    }
  });
}
...
async function runTs24(page, data) {
  ...
  await reportStep('Apply descending order from visible table headers', async () => {
    for (const label of ['Supplier Name', 'Org Id', 'Facility Name', 'Payment Number']) {
      await clickHeaderAndChoose(page, label, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.menus.descending);
    }
  });
}
...
async function runTs25(page, data) {
  ...
  await reportStep('Apply hide-column from visible table headers', async () => {
    for (const label of ['Supplier Name', 'Org Id']) {
      await clickHeaderAndChoose(page, label, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.menus.hideColumn);
    }
  });
}
...
async function runTs28(page, data) {
  ...
  await reportStep('Scroll down and return to the top of the payment list', async () => {
    ...
    await waitForAppToSettle(page, 500);
    await clickWithFallback(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.list.returnToTop, {
      mustBeVisible: true,
      timeoutPerCandidate: 2500
    });
    await expect((await resolveFirst(page, imremitDashboardFailedPaymentsOnProviderPaymentProviderErrorSelectors.headings.paymentManagement, {
      mustBeVisible: true,
      timeoutPerCandidate: 5000
...
async function runScenario(page, data, scenarioName) {
  ...
  if (!scenarioKey) {
    throw new Error(`imREmit Dashboard Failed Payments on Provider/Payment Provider Error scenario not implemented: ${scenarioName}`);
  }
