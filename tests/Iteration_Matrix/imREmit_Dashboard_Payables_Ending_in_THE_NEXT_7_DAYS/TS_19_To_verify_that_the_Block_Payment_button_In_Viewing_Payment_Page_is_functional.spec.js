...
test("TS_19_To_verify_that_the_Block_Payment_button_In_Viewing_Payment_Page_is_functional", async ({ page }) => {
  const data = loadRuntimeData();
  test.info().annotations.push({
    type: 'source-aiq',
    description: "source-aiq/Test Scripts/imREmit_Dashboard/Payables_Ending_in_THE_NEXT_7_DAYS/TS_19_To_verify_that_the_Block_Payment_button_In_Viewing_Payment_Page_is_functional.ds"
  });
  ...
  await test.step('Open the Block Payment action in payment details', async () => {
    await imremitDashboardPayablesEndingInTheNext7DaysHelpers.clickDetailButton(
      page,
      imremitDashboardPayablesEndingInTheNext7DaysHelpers.selectors.detail.blockPayment
    );
  });
