...
const modulePath = 'tests/Iteration_Matrix';
const moduleDir = path.join(rootDir, modulePath);
const resultsDir = path.join(rootDir, 'allure-results');
const shareableDirRelative = path.join('Result', 'allure-report-iteration-matrix-shareable');
const zipPathRelative = path.join('Result', 'iteration-matrix-full-project.zip');
...
function writeMetadataFiles() {
  ...
  const environment = [
    ...
    'Scope=Last-failed rerun merged into Iteration Matrix full baseline',
    `Generated=${today}`,
  ].join('\n');
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: 'GitHub Copilot',
      type: 'local',
      buildName: 'Iteration Matrix last-failed client report',
      buildOrder: 1,
...
function main() {
  const baselineBackupDir = createBackup(resultsDir, 'iteration-matrix-allure-');
