const mgmtPaymentReceivedSelectors = {
  navigation: {
    ...
    paymentsReceivedTab: [
      { type: 'role', role: 'tab', options: { name: 'Payments Received', exact: true }, name: 'payments received tab' },
      { type: 'text', value: 'Payments Received', options: { exact: true }, name: 'payments received text' }
    ],
    paymentsReceivedHeading: [
      { type: 'role', role: 'heading', options: { name: 'MIS Payments Received', exact: true }, name: 'payments received heading' },
      { type: 'text', value: 'MIS Payments Received', options: { exact: true }, name: 'payments received text heading' }
    ]
  },
  page: {
    graphHeading: [
      { type: 'role', role: 'heading', options: { name: 'Payments Received Graph', exact: true }, name: 'graph heading' },
      { type: 'text', value: 'Payments Received Graph', options: { exact: true }, name: 'graph heading text' }
    ],
    adjustFiltersButton: [
    ...
    pageResetFiltersButton: [
      ...
      { type: 'custom', name: 'page reset filters button fallback', factory: (page) => page.locator('button').filter({ hasText: /^Reset Filters$/ }).first() }
    ],
    monthlyPaymentsLegend: [
      { type: 'text', value: 'Monthly Payments', options: { exact: true }, name: 'monthly payments legend' }
    ],
    trendlineLegend: [
      { type: 'text', value: 'Trendline', options: { exact: true }, name: 'trendline legend' }
    ],
    ...
    tableHeading: [
      { type: 'role', role: 'heading', options: { name: 'Payments Received Table', exact: true }, name: 'table heading' },
      { type: 'text', value: 'Payments Received Table', options: { exact: true }, name: 'table heading text' }
    ],
    pageStatus: [
      { type: 'custom', name: 'page status text', factory: (page) => page.locator('text=/^Page:\s*\d+\s+of\s+\d+$/').first() },
      { type: 'custom', name: 'viewing entries text', factory: (page) => page.locator('text=/^Viewing Entries:\s*\d+\s+to\s+\d+$/').first() }
    ],
    tableElement: [
      { type: 'custom', name: 'payments received table', factory: (page) => page.locator('#table-container table').first() },
  ...
  filters: {
    ...
    yearLabel: [{ type: 'text', value: 'Year:', options: { exact: true }, name: 'year label' }],
    moduleLabel: [{ type: 'text', value: 'Module:', options: { exact: true }, name: 'module label' }],
    customersLabel: [
    ...
    selectAllStatusesOption: [{ type: 'text', value: 'Select All Statuses', options: { exact: true }, name: 'select all statuses option' }],
    clearAllButton: [
      { type: 'text', value: 'Clear All', options: { exact: true }, name: 'clear all text' },
      { type: 'custom', name: 'clear all button', factory: (page) => page.locator('[role="dialog"] [aria-label="Clear all selections"], [role="dialog"] button, [role="dialog"] span, [aria-label="Clear all selections"]').filter({ hasText: /^Clear All$/ }).first() }
    ],
    resetFiltersButton: [
      ...
      { type: 'custom', name: 'reset filters button fallback', factory: (page) => page.locator('button').filter({ hasText: /^(Reset|Reset Filters)$/ }).first() }
    ],
    customerSearchInput: [
