...
const modulePath = 'tests/DigitEYESCamp_Cluster/DigitEYESCamps_Participants';
const moduleDir = path.join(rootDir, modulePath);
const resultsDir = path.join(rootDir, 'allure-results');
const shareableDirRelative = path.join('Result', 'allure-report-camp-cluster-participants-shareable');
const zipPathRelative = path.join('Result', 'digit-eyes-camp-cluster-participants.zip');
...
function writeMetadataFiles() {
  ...
  const environment = [
    ...
    'Track=DigitEYESCamp Cluster',
    'Module=DigitEYESCamps Participants',
    'Scope=4 participant module scripts with AIQ-linked descriptions',
    `Generated=${today}`,
  ].join('\n');
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: 'GitHub Copilot',
      type: 'local',
      buildName: 'DigitEYESCamp Cluster Participants client report',
      buildOrder: 1,
