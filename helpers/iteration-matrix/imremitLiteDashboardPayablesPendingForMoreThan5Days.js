...
async function openModule(page) {
  const result = await clickIfFound(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.moduleTabs.imremit, {
    mustBeVisible: true,
...
async function openDashboard(page) {
  await clickWithFallback(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.moduleTabs.dashboard, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  });
  await waitForAppToSettle(page, 1000);
...
async function selectCustomer(page, data, options = {}) {
  ...
  let trigger;
  try {
    trigger = await resolveFirst(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.customerPicker.trigger, {
      mustBeVisible: true,
      timeoutPerCandidate: 2500
    });
  } catch (error) {
  ...
  const searchInput = await resolveFirst(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.customerPicker.searchInput, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  }).catch(() => null);

  if (searchInput && preferredCustomer) {
    await fillWithFallback(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.customerPicker.searchInput, preferredCustomer, {
      mustBeVisible: true,
      timeoutPerCandidate: 2500
    });
    await waitForAppToSettle(page, 250);
    ...
    if (fallbackCustomer) {
      await fillWithFallback(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.customerPicker.searchInput, fallbackCustomer, {
        mustBeVisible: true,
        timeoutPerCandidate: 2500
      });
      await waitForAppToSettle(page, 250);
...
async function expectOverviewVisible(page) {
  const overviewHeading = await resolveFirst(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.headings.overview, {
    mustBeVisible: true,
  ...
  const cardHeading = await resolveFirst(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.dashboard.payablesPendingForMoreThan5DaysHeading, {
    mustBeVisible: true,
    timeoutPerCandidate: 3000
  });
...
async function assertPaymentManagementVisible(page) {
  const heading = await resolveFirst(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.headings.paymentManagement, {
    mustBeVisible: true,
...
async function closeCustomerDetails(page) {
  const closeControl = await resolveFirst(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.dashboard.customerDetailsClose, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  }).catch(() => null);
  ...
  if (closeControl) {
    ...
    await clickWithFallback(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.dashboard.viewCustomerDetails, {
      mustBeVisible: true,
      timeoutPerCandidate: 2500
    });
  }
...
async function openPayablesPendingForMoreThan5DaysList(page, data) {
  await openDashboardWorkspace(page, data);

  const openPaymentManagement = async () => {
    const reviewAction = await resolveFirst(
      page,
      imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.dashboard.review.filter((candidate) => candidate.type !== 'text'),
      {
        mustBeVisible: true,
        timeoutPerCandidate: 2500
      }
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    const visiblePaymentRowAction = await resolveFirst(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.list.actionsMenu, {
      mustBeVisible: true,
...
async function openActionsMenu(page) {
  await clickWithFallback(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.list.actionsMenu, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  });
  await waitForAppToSettle(page, 500);
...
async function openPaymentDetails(page, data) {
  await openPayablesPendingForMoreThan5DaysList(page, data);

  for (let attempt = 0; attempt < 3; attempt += 1) {
    const paymentListButtonBeforeOpen = await resolveFirst(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.detail.paymentList, {
      mustBeVisible: true,
    ...
    const detailsMenuItem = await resolveFirst(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.list.viewPaymentDetails, {
      mustBeVisible: true,
      timeoutPerCandidate: 1500
    }).catch(() => null);
  ...
  const paymentListButton = await resolveFirst(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.detail.paymentList, {
    mustBeVisible: true,
    timeoutPerCandidate: 5000
  }).catch(() => null);
...
async function openPaginationDropdown(page) {
  const dropdown = await resolveFirst(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.list.paginationDropdown, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  });
...
async function choosePaginationOption(page, value) {
  ...
  const option = await resolveFirst(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.list.paginationOption(value), {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  });
...
async function clickBackToDashboard(page) {
  const backToDashboardControl = await resolveFirst(page, [
    ...imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.list.backToDashboard,
    ...imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.moduleTabs.dashboard
  ], {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  });
...
async function applyHeaderActions(page, actionCandidates, labels) {
  ...
  if (!applied) {
    throw new Error('No sortable headers were available for the Payables Pending for More Than 5 Days list.');
  }
}

async function assertBlockPaymentVisible(page) {
  const blockPayment = await resolveFirst(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.detail.blockPayment, {
    mustBeVisible: true,
    timeoutPerCandidate: 2500
  });
...
async function runTs02(page, data) {
  ...
  await reportStep('Open and close customer details from the dashboard', async () => {
...
async function runTs03(page, data) {
  await reportStep('Open the Payables Pending for More Than 5 Days review list', async () => {
    await openPayablesPendingForMoreThan5DaysList(page, data);
  });
}
...
async function runTs08(page, data) {
  await reportStep('Open payment details from the Payables Pending for More Than 5 Days list', async () => {
    await openPaymentDetails(page, data);
  });
  await reportStep('Move to the next payment detail', async () => {
    await baseHelpers.clickDetailButton(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.detail.nextPayment);
  });
}
...
async function runTs09(page, data) {
  ...
  await reportStep('Move to the previous payment detail', async () => {
    ...
    await baseHelpers.clickDetailButton(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.detail.previousPayment);
  });
}
...
async function runTs11(page, data) {
  ...
  await reportStep('Return from payment details to the payments list', async () => {
    await baseHelpers.clickDetailButton(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.detail.paymentList);
    await assertPaymentManagementVisible(page);
  });
}
...
async function runTs14(page, data) {
  ...
  await reportStep('Open invoices in payment details', async () => {
    await baseHelpers.clickDetailButton(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.detail.invoices);
  });
}
...
async function runTs15(page, data) {
  ...
  await reportStep('Open history in payment details', async () => {
    await baseHelpers.clickDetailButton(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.detail.history);
  });
}
...
async function runTs17(page, data) {
  ...
  await reportStep('Use the next-page control when it is enabled', async () => {
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const clicked = await clickPaginationWhenEnabled(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.pagination.next);
      if (!clicked) {
        break;
      }
    }
...
async function runTs18(page, data) {
  ...
  await reportStep('Move forward and then use the previous-page control', async () => {
    await clickPaginationWhenEnabled(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.pagination.next);
    await clickPaginationWhenEnabled(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.pagination.previous);
  });
}
...
async function runTs19(page, data) {
  ...
  await reportStep('Move to the last page and return to the first page when possible', async () => {
    const movedToLast = await clickPaginationWhenEnabled(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.pagination.last);
    if (movedToLast) {
      await clickPaginationWhenEnabled(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.pagination.first);
    }
  });
}
...
async function runTs20(page, data) {
  ...
  await reportStep('Use the last-page control when it is enabled', async () => {
    await clickPaginationWhenEnabled(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.pagination.last);
  });
}
...
async function runTs21(page, data) {
  ...
  await reportStep('Apply ascending order from the visible table headers', async () => {
    await applyHeaderActions(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.menus.ascending, ['Supplier Name', 'Payment Number', 'Sent Date', 'Status Description']);
  });
}
...
async function runTs22(page, data) {
  ...
  await reportStep('Apply descending order from the visible table headers', async () => {
    await applyHeaderActions(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.menus.descending, ['Supplier Name', 'Payment Number', 'Sent Date', 'Status Description']);
  });
}
...
async function runTs23(page, data) {
  ...
  await reportStep('Apply hide-column from the visible table headers', async () => {
    await applyHeaderActions(page, imremitLiteDashboardPayablesPendingForMoreThan5DaysSelectors.menus.hideColumn, ['Supplier Name', 'Payment Number', 'Sent Date', 'Status Description']);
  });
}
...
async function runScenario(page, data, scenarioName) {
  ...
  if (!scenarioKey) {
    throw new Error(`imREmit Lite Dashboard Payables Pending for More Than 5 Days scenario not implemented: ${scenarioName}`);
  }
...
module.exports = {
  imremitLiteDashboardPayablesPendingForMoreThan5DaysHelpers: {
    ...
    verifyAddComment: runTs10,
    verifyPaymentsListButton: runTs11,
    ...
    verifyDescendingSort: runTs22,
    verifyHideSort: runTs23,
    verifyReturnToTopButton: runTs24,
    ...wrapHelperMapWithReadableSteps({
      openModule,
      openDashboard,
      selectCustomer,
      openDashboardWorkspace,
      openPayablesPendingForMoreThan5DaysList,
      openActionsMenu,
      openPaymentDetails,
      ...
      openPayablesPendingForMoreThan5DaysList: 'Open Payables Pending for More Than 5 Days list',
