...
test("TS_20_To_verify_the_sorting_arrows_in_the_entries_desc", async ({ page }) => {
  ...
  test.info().annotations.push({
    type: 'source-aiq',
    description: "source-aiq/Test Scripts/imREmit_Lite_Dashboard/Partial_Payment_Already_Taken/TS_20_To_verify_the_sorting_arrows_in_the_entries_desc.ds"
  });
  ...
  await test.step('Apply descending order from the visible table headers', async () => {
    await imremitLiteDashboardPartialPaymentAlreadyTakenHelpers.applyHeaderActions(page, imremitLiteDashboardPartialPaymentAlreadyTakenSelectors.menus.descending, ['Supplier Name', 'Payment Number', 'Sent Amount']);
  });
