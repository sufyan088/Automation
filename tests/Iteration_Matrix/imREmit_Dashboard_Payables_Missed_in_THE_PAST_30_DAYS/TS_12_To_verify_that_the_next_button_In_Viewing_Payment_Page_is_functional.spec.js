...
test("TS_12_To_verify_that_the_next_button_In_Viewing_Payment_Page_is_functional", async ({ page }) => {
  const data = loadRuntimeData();
  test.info().annotations.push({
    type: 'source-aiq',
    description: "source-aiq/Test Scripts/imREmit_Dashboard/Payables_Missed_in_THE_PAST_30_DAYS/TS_12_To_verify_that_the_next_button_In_Viewing_Payment_Page_is_functional.ds"
  });
  ...
  await test.step('Move to the next payment detail', async () => {
    await imremitDashboardPayablesMissedInThePast30DaysHelpers.clickDetailButton(
      page,
      imremitDashboardPayablesMissedInThePast30DaysHelpers.selectors.detail.nextPayment
    );
  });
